...
namespace Game.Hotfix.Common
{
    ...
    public class DRTower : DataRowBase
    {
        ...
        {
            string[] columnStrings = dataRowString.Split(DataTableReader.DataSplitSeparators);
            for (int i = 0; i < columnStrings.Length; i++)
            {
                columnStrings[i] = columnStrings[i].Trim(DataTableReader.DataTrimSeparators);
            }
            ...
            MaxHP = float.Parse(columnStrings[index++]);
                Dimensions = DataTableReader.ParseInt32Array(columnStrings[index++]);
            Type = columnStrings[index++];
                Levels = DataTableReader.ParseInt32Array(columnStrings[index++]);
