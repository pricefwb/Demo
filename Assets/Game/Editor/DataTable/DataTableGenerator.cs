...
namespace Game.Editor.DataTableTools
{
    ...
    {
        public readonly static string DataTablePath = "Assets/Game/Hotfix/DataTables";
        public readonly static string CSharpCodePath = "Assets/Game/Hotfix/Scripts/Common/DataTable/DR";
        public readonly static string CSharpCodeTemplateFileName = "Assets/Game/Editor/Configs/DataTableCodeTemplate.txt";
        private readonly static string CSharpCodeNameSpace = "Game.Hotfix.Common";
        private static readonly Regex EndWithNumberRegex = new Regex(@"\d+$");
        private static readonly Regex NameRegex = new Regex(@"^[A-Z][A-Za-z0-9_]*$");

        public static string[] GetDataTableNames()
        {
            //string dataTablesPath = Application.dataPath + @"/Game/Hotfix/DataTables";

            DirectoryInfo directoryInfo = new DirectoryInfo(DataTablePath);
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            ...
            stringBuilder
                ...
                .AppendLine("        {")
                .AppendLine("            string[] columnStrings = dataRowString.Split(DataTableReader.DataSplitSeparators);")
                .AppendLine("            for (int i = 0; i < columnStrings.Length; i++)")
                .AppendLine("            {")
                .AppendLine("                columnStrings[i] = columnStrings[i].Trim(DataTableReader.DataTrimSeparators);")
                .AppendLine("            }")
                .AppendLine()
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else if (languageKeyword.Contains("[]"))
                {
                    ...
                    stringBuilder.AppendFormat("                {0} = DataTableReader.Parse{1}(columnStrings[index++]);", dataTableProcessor.GetName(i), readFunctionName).AppendLine();
                }
                else
                {
                    stringBuilder.AppendFormat("            {0} = DataTableReader.Parse{1}(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                }
            }
